...
from config.config import Config, MODE, SUPPORTED_MODELS
...
class Trainer:
    ...
    def train(self):
        with logging_redirect_tqdm([self.logger]):
            self.logger.info("Config:\n" + Config.__list_config__(self.cfg))
        # export cfg with pickle
        Config.__save_config__(self.cfg, os.path.join(self.log_dir, "cfg.pickle"))
    ...
    def build_model(self):
        ...
        if model == "UNet":
            ...
            model = Unetpp(
                model_cfg.unpp.input_channels,
                model_cfg.unpp.base_num_features,
                model_cfg.unpp.num_classes,
                model_cfg.unpp.num_pool,
                model_cfg.unpp.profile_size,
            )
        elif model == "AsymUNet":
            model = AsymUNet(
        ...
        if self.cfg.mode == "train" and self.cfg.train_cfg.checkpoint:
            model.load_state_dict(
                torch.load(self.cfg.train_cfg.checkpoint, map_location="cpu")
            )
            with logging_redirect_tqdm([self.logger]):
                self.logger.info(
                    f"Pretrained model is loaded from {self.cfg.train_cfg.checkpoint}"
                )
        elif self.cfg.mode in ["eval", "infer"]:
            model.load_state_dict(
                torch.load(self.cfg.eval_cfg.infer_weight_path, map_location="cpu")
            )
            tqdm.write(f"Loaded model from {self.cfg.eval_cfg.infer_weight_path}")
            model.eval()
    ...
    def create_acc_metric(cfg):
        acc_metric = PixelwiseAccuracy(cfg.acc_cfg)
    ...
    def train_epoch(
        ...
        for i, data in tqdm(
            ...
            with torch.autocast(
                device_type=cfg.device.type, dtype=torch.float16, enabled=cfg.amp
            ):
                # forward
    ...
    def valid_epoch(
        ...
        for i, data in tqdm(
            ...
            inputs, labels = data
            inputs, labels = inputs.to(cfg.device.type), labels.to(cfg.device)
            with torch.no_grad():
                with torch.autocast(
                    device_type=cfg.device.type, dtype=torch.float16, enabled=cfg.amp
                ):
