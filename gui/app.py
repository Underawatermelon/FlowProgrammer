...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, cfg, *args, obj=None, **kwargs):
        ...
        self.obs_pos_fin = [False for i in range(self.num_obs)]  # True for Top Wall.

        # the profile imgs, from input image to output image, RGB float 0~1
        self.profiles_fin = np.zeros((self.num_obs + 1, self.res[0], self.res[1], 3), dtype=np.uint8)
    ...
    def draw_pin(self, param, widget):
        pin_mat = gen_pin_tensor(param, self.res)
        ...
        try:
            ...
            try:
                with open(fname[0], "r") as f:
                    obs_no, coords, pos = self.read_obs_param(f)
        ...
        try:
            ...
            try:
                with Timer() as t:
                    ...
                    with open(fname[0], "r") as f:
                        while True:
                            # read until the read_obs_param returns None
                            obs_no, coords, pos = self.read_obs_param(f)
                            if obs_no is None:
                                break
                            obs_no_list.append(obs_no)
                            self.obs_coord_fin[obs_no - 1] = coords
                            self.obs_pos_fin[obs_no - 1] = pos
                    ...
                    print(f"loaded obstacles: {obs_no_list if obs_no_list else None}")
            except ValueError:
                print("obstacle info is incorrect.")
            except KeyError:
                print(f"obstacle number is incorrect: {obs_no}")
            except:
        ...
        try:
            ...
            try:
                with open(fname[0], "w") as f:
                    self.write_obs_param(f, 0, self.obs_coord_tmp, self.obs_pos_tmp)
    ...
    def export_TT_plot(self):
        ...
        try:
            assert ok
            tt_no_list = np.fromstring(text, dtype=np.int8, sep=",")
        ...
        try:
            for i in tqdm(tt_no_list):
                ...
                plot_tt_vf(self.tt_fin[i - 1], ax, plot_res=self.res)
