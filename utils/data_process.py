...
def gen_pin_tensor(
    ...
    """generate the input profile matrix and plot. return the img matrix
    Three color bands in three channel.
    Args:
    ...
    if len(param) == 2:
        pin_mat = np.zeros((pf_res[0], pf_res[1], 1), dtype=np.uint8)
        pin_mat[param[0] : param[1]] = 255  # G
    else:
        pin_mat = np.zeros((pf_res[0], pf_res[1], 3), dtype=np.uint8)
...
def obs_params2imgs(obs_coords, obs_pos=False, ax=None):
    if ax is None:
        flag_close_fig = True
...
def tt_synth(tt_list: list) -> np.ndarray:
    ...
    result_tt = coord_tans - grids
    result_tt = result_tt.astype(np.int16)
    return result_tt
...
class InflowCalculator:
    ...
    def set_flow_rates(self, fr1, fr2, fr3):
        ...
        Args:
            fr11 (float): flow rate of stream 1
